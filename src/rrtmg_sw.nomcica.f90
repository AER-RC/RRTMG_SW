...
! Input arrays from GCM
      ...
      real(kind=jprb), intent(in) :: taucld(nbndsw,nlon,mxlay)  ! Cloud optical depth
      real(kind=jprb), intent(in) :: ssacld(nbndsw,nlon,mxlay)  ! Cloud single scattering albedo
      real(kind=jprb), intent(in) :: asmcld(nbndsw,nlon,mxlay)  ! Cloud asymmetry parameter
...
! other input parameters
         ...
         call inatm_sw (iplon, icld, iaer, play, plev, tlay, tlev, tsfc, &
              ...
              inflgsw, iceflgsw, liqflgsw, cldfr, taucld, ssacld, asmcld, &
              cicewp, cliqwp, reice, reliq, tauaer, ssaaer, asmaer, &
              nlayers, pavel, pz, pdp, tavel, tz, tbound, coldry, wkl, &
              adjflux, solvar, inflag, iceflag, liqflag, cldfrac, tauc, ssac, &
              asmc, ciwp, clwp, rei, rel, taua, ssaa, asma)
...
! Clouds
         if (icld.eq.0) then
            ztauc(:mxlay,:nbndsw) = 0._jprb
            ztaucorig(:mxlay,:nbndsw) = 0._jprb
            zasyc(:mxlay,:nbndsw) = 0._jprb
            zomgc(:mxlay,:nbndsw) = 0._jprb
...
! Input arrays from GCM
      ...
      real(kind=jprb), intent(in) :: cldfr(nlon,mxlay)          ! Cloud fraction
      real(kind=jprb), intent(in) :: taucld(nbndsw,nlon,mxlay)  ! Cloud optical depth (optional)
      real(kind=jprb), intent(in) :: ssacld(nbndsw,nlon,mxlay)  ! Cloud single scattering albedo
      real(kind=jprb), intent(in) :: asmcld(nbndsw,nlon,mxlay)  ! Cloud asymmetry parameter
      real(kind=jprb), intent(in) :: cicewp(nlon,mxlay)         ! Cloud ice water path (g/m2)
      real(kind=jprb), intent(in) :: cliqwp(nlon,mxlay)         ! Cloud liquid water path (g/m2)
...
!         ssac(:nbndsw,nlayers) = 0.0_jprb
!         asmc(:nbndsw,nlayers) = 0.0_jprb
